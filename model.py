...
class CNN:
    ...
    def build_net(self):
        ...
        net = maxpool(net, name = "max_pool1")
        ...
        net = maxpool(net, name = "max_pool2")
    ...
    def training_run(self,batch_size,num_epoch, multiple):
        ...
        with tf.Session() as sess:
            ...
            if not os.path.exists(os.getcwd() + self.save_dir[1:]):
                os.makedirs(os.getcwd() + self.save_dir[1:])
            recent_ckpt_job_path = tf.train.latest_checkpoint(self.save_dir)

            if recent_ckpt_job_path is not None:
                sess.run(tf.initialize_all_variables())
                coord = tf.train.Coordinator()
                threads = tf.train.start_queue_runners(sess=sess, coord=coord)
                print("\nWe find ckpt name: ", recent_ckpt_job_path)
                print("restore parameters...")
                saver.restore(sess,recent_ckpt_job_path)
                print("complete to restore!\n")
                base_name = os.path.basename(recent_ckpt_job_path)
            ...
            for i in range(counter,num_epoch,1):
                ...
                if ((i % 5 == 0) or (i == (num_epoch -1))):
                    ckpt_path = saver.save(sess,self.save_dir + "/parameter",i)
